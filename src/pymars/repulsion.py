...
def setup_repulsion_potential(target_species, bullet_species):
    """Setup a ZBL repulsion potential between bullets and target atoms.

    Args:
        - target_species: array of shape (N,) with target atomic species indices
        - bullet_species: atomic species index of the bullets
    Returns:
    ...
    def repulsion_energies_and_forces(coordinates, bullet_coordinates):
        """Compute ZBL repulsion energies and forces.
        Args:
            - coordinates: array of shape (batch_size,N,3) with target atomic coordinates
            - bullet_coordinates: array of shape (batch_size,3) with bullet coordinates
        Returns:
            ...
            - bullet_forces: array of shape (batch_size,3) with repulsion forces on bullets
        """
        vecs = coordinates - bullet_coordinates[:,None,:]  # (batch_size,N,3)
        ...
        bullet_forces = -np.sum(forces,axis=1)  # (batch_size,3)

        return energies, forces, bullet_forces
